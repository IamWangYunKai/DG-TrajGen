...
class DrivingBenchmark(object):
    ...
    def __init__(
        ...
        print('save name: ' + str(name_to_save))
    ...
    def benchmark_agent(self, experiment_suite, agent, client):
        ...
        logging.info('START')
        print('START')
        print('len: '+str(len(experiment_suite.get_experiments()[int(start_experiment):])))

        for i, experiment in enumerate(experiment_suite.get_experiments()[int(start_experiment):]):
            ...
            print('experiment iter: '+str(i))

            for j, pose in enumerate(experiment.poses[start_pose:]):
                print('      pose iter: '+str(j))
                for rep in range(experiment.repetitions):
                    print('        rep iter: '+str(rep))
...
def run_benchmark(world, vehicle, agent, sensor_manager, experiment_suite):
    for i, experiment in enumerate(experiment_suite.get_experiments()):
        run_experiment(world, vehicle, agent, sensor_manager, experiment)


def run_experiment(world, vehicle, agent, sensor_manager, experiment):
    for j, pose_tuple in enumerate(experiment.poses):
        if j < 1:
            continue
        for rep in range(experiment.repetitions):
            run_episode(world, vehicle, agent, sensor_manager, pose_tuple)



import carla_utils as cu
import carla
from carla_utils.system import printVariable
import pygame

def run_episode(world, vehicle, agent, sensor_manager, pose_tuple, time_out=30.0):
    start_transform, end_transform = pose_tuple[0], pose_tuple[1]
    start_transform = carla.Location(x=320.8699645996094, y=327.04998779296875, z=0)
    printVariable('start_transform', start_transform)
    printVariable('end_transform', end_transform)

    cu.respawn_vehicle(world.get_map(), vehicle, start_transform.location)
    success_distance = 5.0 # [m]
    ...
    while (current_timestamp - initial_timestamp) < time_out:
        ...
        if vehicle.get_location().distance(end_transform.location) < success_distance:
            return
        current_timestamp = time.time()

        printVariable('location', vehicle.get_location())
        control = agent.run_step(debug=True)
        control.manual_gear_shift = False
        vehicle.apply_control(control)

        # visualization
        image = sensor_manager['camera'].get_data()
        image = image[:, :, :3]
        image = image[:, :, ::-1]
