...
parser.add_argument('--dataset_name', type=str, default="fine-tune-01", help='name of the dataset')
...
parser.add_argument('--test_interval', type=int, default=3, help='interval between model test')
...
if not opt.test_mode:
    logger_kt = SummaryWriter(log_dir=log_path+'/irm/kitti/')
    logger_rbk = SummaryWriter(log_dir=log_path+'/irm/robotcar/')
    logger_cl = SummaryWriter(log_dir=log_path+'/irm/carla/')
...
eval_loader3 = DataLoader(CARLADataset(data_index=list(range(1,10)), opt=opt, dataset_path='/media/wang/DATASET/CARLA_HUMAN/town01/'), batch_size=1, shuffle=False, num_workers=1)
...
def eval_error(encoder, logger, total_step):
    kitti_points_num = 10
    encoder.eval()
...
def eval_error_robotcar(encoder, logger, total_step):
    points_num = 16
    encoder.eval()
...
def eval_error_carla(encoder, logger, total_step):
    points_num = 16
    encoder.eval()
...
def get_latent(real_traj, generator, use_irm=True):
    single_latent = torch.randn(opt.batch_size, 1, opt.vector_dim).to(device)
    single_latent.requires_grad = True

    optimizer = torch.optim.Adam(params=[single_latent], lr=1e-1)
    single_latent_best = single_latent.clone()
    loss_best = torch.ones(()).to(single_latent.device) * 1000.0
    for _ in range(3000):
        optimizer.zero_grad()
        ...
        loss_l2 = 0.01*torch.norm(single_latent)
        ...
        loss = loss_trajectory + loss_l2
        if use_irm:
            train_penalty = 0.1*grad_penalty(loss_trajectory, generator)
            loss = loss_trajectory + train_penalty + loss_l2
...
for i, batch in enumerate(train_loader):
    ...
    single_latent_best = get_latent(real_traj, generator)
    single_latent = encoder(batch['img'], batch['v_0'])
    single_latent = single_latent.unsqueeze(1)

    loss_latent = trajectory_criterion(single_latent_best, single_latent)
    ...
    logger_rbk.add_scalar('train/loss_latent', loss_latent.item(), total_step)

    if total_step % opt.test_interval == 0:
        eval_error(encoder, logger_kt, total_step)
        eval_error_robotcar(encoder, logger_rbk, total_step)
        eval_error_carla(encoder, logger_cl, total_step)
