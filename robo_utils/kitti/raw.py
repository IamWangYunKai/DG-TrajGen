...
class Raw:
    ...
    imu_height = 0.93   # imu height w.r.t. ground
    ...
    def __init__(self, base_path, date, drive, **kwargs):
        ...
        self.imu_height = Raw.imu_height
        self.pose_array = self._get_pose_array()
        self.lengths = basic_utils.get_trajectory_length_array(self.pose_array)
        try:
            self.trajectory_types = np.loadtxt(join(self.data_path, 'traj_type.txt'), delimiter=' ')
        except IOError:
            self.trajectory_types = np.array([]).reshape(0,2)
    ...
    def length_valid(self):
        return len(self.costmap_files)

    def _get_pose_array(self):
        '''
        Returns
            numpy.array 6 × len(dataset), xyzrpy
        '''
        homogeneous = cu.basic.RotationMatrixTranslationVector.homogeneous_matrix
        euler = cu.basic.Euler.rotation_matrix

        R, t = homogeneous(self.oxts[0].T_w_imu)
        rpy = euler(R)
        pose_array = np.vstack((t, np.array(rpy).reshape(3,1)))
        for oxts in self.oxts[1:]:
            R, t = homogeneous(oxts.T_w_imu)
            rpy = euler(R)
            pose = np.vstack((t, np.array(rpy).reshape(3,1)))
            pose_array = np.hstack((pose_array, pose))
        '''minus the height of imu'''
        pose_array[2,:] -= self.imu_height
        # z = min(pose_array[2,:]) - self.imu_height
        # pose_array[2,:] = z
        return pose_array
        
    def get_image_pm(self, index):
        # return utils.load_image(self.image_pm_files[index], mode='L')
        return cv2.imread(self.image_pm_files[index])

    def get_costmap(self, index):
        return utils.load_image(self.costmap_files[index], mode='L')
    ...
    def get_trajectory_type(self, index):
        if self.trajectory_types.shape[0] <= 0:
            return None
        return self.trajectory_types[index,1]
    
    def get_image(self, index):
        image = self.get_cam2(index)
        image = image.crop([250, 0, 990, 370])
        image = image.resize((400, 200))
        return image

    def get_nav(self, index):
        nav = utils.load_image(self.nav_files[index], mode='RGB')
        nav = nav.crop([250, 0, 990, 370])
        nav = nav.resize((400, 200))
        return nav
    
    def get_pose_array(self, index, trajectory_length):
        horizon = basic_utils.find_desired_length(self.lengths, index, trajectory_length)
        # print('horizon', len(self.lengths))
        if horizon <= 0:
            print('no enough trajectory length 2', len(self.lengths))
            horizon = 0
        return self.pose_array[:,index:index+horizon]
    ...
    def _get_file_lists(self):
        ...
        self.image_pm_files = sorted(glob.glob( join(self.data_path, 'image_pm',  '*.{}'.format(self.imtype)) ))
        self.costmap_files = sorted(glob.glob( join(self.data_path, 'costmap',  '*.{}'.format(self.imtype)) ))
        self.nav_files = sorted(glob.glob( join(self.data_path, 'nav_map',  '*.{}'.format(self.imtype)) ))
        ...
        if self.frames is not None:
            ...
            self.image_pm_files = utils.subselect_files(self.image_pm_files, self.frames)
            self.costmap_files = utils.subselect_files(self.costmap_files, self.frames)
            self.nav_files = utils.subselect_files(self.nav_files, self.frames)
